  ...
  return (
    <section className="text-[#0A0D17] font-['Roboto']">
      ...
      <div className="bg-white py-8 px-4 md:px-16">
        ...
        <div className="flex flex-row gap-6 overflow-x-auto sm:grid sm:grid-cols-2 lg:grid-cols-3 sm:overflow-visible max-w-6xl mx-auto scrollbar-hide">
          {topic1Items.map((item, index) => (
            <div
              key={index}
              className="bg-white rounded-[32px] p-8 transition duration-300 min-w-[100vw] sm:min-w-0"
            >
      ...
      {/* Topic 02 */}
      <div className="bg-[#444444] py-8 px-4 md:px-45 text-white">
        ...
        <div className="overflow-x-auto scrollbar-hide">
          <div className="flex gap-6 px-2 md:px-0 w-max">
            {topic2Items.map((item, index) => (
              <div
                key={index}
                className="bg-[#444444] rounded-[32px] p-8 transition duration-300 min-w-[100vw] sm:min-w-0"
              >
      ...
      <div className="bg-white py-8 px-4 md:px-0">
        ...
        <div className="flex flex-row gap-6 overflow-x-auto sm:grid sm:grid-cols-2 lg:grid-cols-3 sm:overflow-visible max-w-6xl mx-auto scrollbar-hide">
          {topic3Items.map((item, index) => (
            <div
              key={index}
              className="bg-white rounded-[32px] md:w-[400px] p-8 transition duration-300 min-w-[100vw] sm:min-w-0"
            >
              <img
                src={item.image}
                alt={item.title}
                className="w-[120vw] sm:w-full md:w-[500px] h-[440px] object-cover rounded-[24px] mb-6"
              />
              <h3 className="text-[20px] font-bold mb-2">{item.title}</h3>
